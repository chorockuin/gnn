...
"""
문제 1: ENZYMES 데이터 집합의 class 수와 feature 수는 얼마입니까? (5점)
"""
def get_num_classes(pyg_dataset):
  ...
  num_classes = len(torch.unique(pyg_dataset.data.y))
...
def get_graph_num_edges(pyg_dataset, idx):
  ...
  # pyg_dataset[200]는 sub graph
  # x=[29,3]
  # 3차원 feature를 가진 node 29개가 있음
  #
  # edge_index=[2,106]
  # (시작 node index(0~28), 끝 node index(0~28)) 이렇게 쌍이 106개 있음
  #
  # y=[1]
  # 이 sub graph의 label
  #
  # pyg_dataset[idx].num_edges 이렇게 num_edge를 구할 수 있는데,
  # undirect graph라고 가정하고, u->v v->u는 1개로 계산하랜다
  edge_index = []
  for i in pyg_dataset[idx].edge_index.T:
    s = i[0].item()
    t = i[1].item()
    if [s,t] not in edge_index and [t,s] not in edge_index:
